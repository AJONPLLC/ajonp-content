...
Server Side Rendering(SSR) is a popular technique for rendering a normally client-side only single page app (SPA) on the server and then sending a fully rendered page to the client. - From [alligator.io](https://alligator.io/react/server-side-rendering/)

SSR is not to be confused with Static Site Generators (SSG) like [Hugo](https://gohugo.io/) or [Gatsby](https://www.gatsbyjs.org/) where it generates all of your sites pages in a build step (not dynamically on the server).

Typically a React application is run only on the client side, this is where Next.js comes into play.

[Next.js SSR Benefits](https://nextjs.org/features/server-side-rendering#benefits) include:
...
You are telling Firebase Hosting how to direct traffic:
...
These cloud functions will be found in our `dist/functions` directory.

[/firebase.json](https://github.com/AJONPLLC/ajonp-ajsbooks-nextjs/blob/07-Book-Navigation/firebase.json)
### Full code
```json
{
  "firestore": {
...
1. firebase:serve > Starts the build process
1. firebase:build ->
...
1. firebase serve ->  you can run everything locally to test

1. firebase:deploy is the same as firebase:serve but actaully deploys all of your code.

If nothing else you can copy and paste and all should work 🙀.
...
You can now build a static site for all of the components and you are ready to convert them into lambdas.
Navigate to `books` when selecting one it will open a template.
